...
def GetGalleryTitle(data,gallerytype):
    ...
    if (gallerytype==3):
        _galtitlepre = PageTitle[:len(PageTitle)-15]
    else:
        q = PageTitle.find(' (Page')
        if (q!=-1):
            _galtitlepre = PageTitle[13:q]
        else:
            _galtitlepre = PageTitle

    ### remove forbidden characters so the title can be
    ### used as folder name
    gallerytitle=''
    _galtitle = _galtitlepre.encode('utf-8')

    char_allowed = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789#+~.,-_^!�$%&()={}[]@ '
...
def DownloadGallery():
    while (len(queue.queue)>0):
        ...
        Gal_Url = queue.queue[0][0]
        urltype = queue.queue[0][1]
        destination_folder = queue.queue[0][2]
